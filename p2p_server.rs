...
impl P2PPluginServer {
    ...
    fn parse_verb(&self, msg: &str) -> Result<P2PVerb, Box<dyn std::error::Error>> {
        ...
        match parts[0] {
            ...
            // Telemetry verbs
            "START_TRACE" => Ok(P2PVerb::StartTrace(parts[1].to_string(), parts[2].to_string())),
            "STOP_TRACE" => Ok(P2PVerb::StopTrace(parts[1].to_string())),
            "PERF_RECORD" => Ok(P2PVerb::PerfRecord(parts[1].to_string(), parts[2..].join(" "))),
            "STRACE_RECORD" => Ok(P2PVerb::StraceRecord(parts[1].to_string(), parts[2..].join(" "))),
            "COMPARE_TRACES" => Ok(P2PVerb::CompareTraces(parts[1].to_string(), parts[2].to_string())),
            // Data manipulation verbs
            "INTERCEPT" => Ok(P2PVerb::Intercept(parts[1].to_string(), parts[2].to_string())),
            "EXTRACT" => Ok(P2PVerb::Extract(parts[1].to_string(), parts[2].to_string())),
            "EXFILTRATE" => Ok(P2PVerb::Exfiltrate(parts[1].to_string(), parts[2].to_string())),
