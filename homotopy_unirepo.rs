...
impl HomotopyUnirepo {
    ...
    fn extract_repo_objects(&mut self, repo_path: &std::path::Path, repo_name: &str) -> Result<(), String> {
        ...
        for line in output_str.lines() {
            ...
            if parts.len() >= 3 {
                ...
                if self.object_hashes.insert(hash.clone()) {
                    ...
                    self.homotopy_classes.entry(class_key)
    ...
    fn extract_delta_patterns(&self, repo_path: &std::path::Path) -> Result<Vec<String>, String> {
        ...
        for pack_file in pack_files.lines().take(3) { // Sample first 3 packs
    ...
    fn print_homotopy_analysis(&self) {
        ...
        println!("  Unique objects (after deduplication): {}", self.deduplicated_objects);
        println!("  Duplicate objects removed: {}", self.total_objects - self.deduplicated_objects);
        ...
        let mut class_sizes: Vec<_> = self.homotopy_classes.iter()
        ...
        println!("  Total unique object size: {} MB", total_size / 1024 / 1024);

        // Theoretical unirepo benefits
        let space_saved = (self.total_objects - self.deduplicated_objects) as f64 / self.total_objects as f64;
