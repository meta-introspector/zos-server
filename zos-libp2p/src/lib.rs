...
impl UnifiedP2PServer {
        ...
        match verb {
            ...
            P2PVerb::CompileAndLoad(name, source) => {
                ...
                if compile_result.success {
                    ...
                    Ok(format!(
                        "🔨🔗 Compiled and loaded {}: {} symbols",
                        name, load_result
                    ))
                } else {
                    Ok(format!(
                        "❌ Compilation failed for {}: {:?}",
                        name, compile_result.errors
                    ))
                }
            }
            ...
            P2PVerb::SeedDataset(dataset) => {
                ...
                self.datasets
                    .insert(dataset.dataset_name.clone(), dataset.clone());
                Ok(format!("🌱 Seeded dataset: {}", dataset.dataset_name))
            }

            P2PVerb::GenerateParquet(dataset_name, output_path) => {
                let result = self
                    .generate_parquet_dataset(&dataset_name, &output_path)
                    .await?;
                Ok(format!(
                    "📦 Generated Parquet: {} -> {}",
                    dataset_name, result
            ...
            P2PVerb::PublishToHF(dataset_name, hf_repo) => {
                let result = self
                    .hf_manager
                    .publish_dataset(&dataset_name, &hf_repo)
                    .await?;
                Ok(format!(
    ...
    pub fn get_peer_capabilities(&self, peer_id: &str) -> Option<&Vec<String>> {
        self.peers
            .get(peer_id)
            .map(|p| &p.mathematical_capabilities)
    }
        ...
        if let Some(rustc_so) = self.loaded_sos.get("rustc_driver") {
            unsafe {
                ...
                if let Ok(main_func) =
                    rustc_so
                        .library
                        .get::<unsafe extern "C" fn(i32, *const *const i8) -> i32>(
    ...
    fn find_call_only_functions(&self) -> Result<(), Box<dyn std::error::Error>> {
        ...
        if let Some(rustc_so) = self.loaded_sos.get("rustc_driver") {
            ...
            println!(
                "📊 Analyzing {} symbols for call-only patterns",
                rustc_so.symbols.len()
            );
...
"#;
    ...
    // Compile and load .so
    let result = server
        .execute_verb(P2PVerb::CompileAndLoad(
            "math_lib".to_string(),
            rust_code.to_string(),
        ))
        .await?;
    ...
    let result = server
        .execute_verb(P2PVerb::LoadRustcDriver(rustc_path.to_string()))
        .await?;
    println!("{}", result);
    ...
    // Load Cargo
    let result = server
        .execute_verb(P2PVerb::LoadCargo("cargo".to_string()))
        .await?;
    println!("{}", result);

    // Test Cargo version
    let result = server
        .execute_verb(P2PVerb::CallCargoMain(vec![
            "cargo".to_string(),
            "--version".to_string(),
        ]))
        .await?;
