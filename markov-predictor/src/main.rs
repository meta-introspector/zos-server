...
impl DirectoryMarkovPredictor {
    ...
    fn train_on_directory(&mut self, dir: &Path) -> Result<(), Box<dyn std::error::Error>> {
        ...
        println!("Trained on {} files, {} paths", filenames.len(), paths.len());
        Ok(())
    }

    fn collect_files(&self, dir: &Path, filenames: &mut Vec<String>, paths: &mut Vec<String>) -> Result<(), Box<dyn std::error::Error>> {
    ...
    fn predict_similar_paths(&self, count: usize) -> Vec<String> {
        (0..count)
            .map(|_| self.path_chain.generate(8))
            .collect()
    }
}
