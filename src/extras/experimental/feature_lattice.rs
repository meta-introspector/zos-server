...
impl FeatureLattice {
    ...
    pub fn build_lattice(&mut self) {
        ...
        let deps = vec![
            ...
            (
                "bootstrap-self",
                vec!["self-build", "eigenmatrix", "orbit-system"],
            ),
        ];
    ...
    pub fn prove_orbit_warping(&self) -> String {
        ...
        proof.push_str(
            "Each Rust feature flag f ∈ F creates a measurable warp W(f) in the orbit space Ω.\n\n",
        );
        ...
        proof.push_str("## QED\n");
        proof.push_str(
            "Since W(f) > 0 for all tested features, each feature demonstrably warps the orbit.\n",
        );
