...
impl BlockchainIngestor {
    pub async fn new() -> Result<Self, Box<dyn std::error::Error>> {
        ...
        let rollup_coordinator = RollupCoordinator {
            ethereum_plugin: EthereumPlugin::new(
                "/nix/store/.../lib/zos-plugins/ethereum_plugin.so",
            )?,
    ...
    pub async fn start_ingestion(&mut self) -> Result<(), Box<dyn std::error::Error>> {
        ...
        for (chain_name, config) in &self.supported_chains {
            ...
            self.start_chain_ingestion(chain_name.clone(), config.clone())
                .await?;
        }
    ...
    async fn start_chain_ingestion(
        &self,
        chain_name: String,
        config: ChainConfig,
                ...
                match Self::fetch_block(&config, current_block).await {
                    ...
                    Err(e) => {
                        eprintln!(
                            ...
                            chain_name, current_block, e
                        );
                        tokio::time::sleep(tokio::time::Duration::from_secs(config.block_time))
                            .await;
                    }
                }
    ...
    async fn create_zos_rollup(
        ...
        println!(
            "✅ ZOS rollup created and shared with {} nodes",
            self.zos_nodes.len()
        );
        Ok(())
    }
    ...
    async fn share_rollup_with_nodes(
        &self,
        rollup_proof: String,
        zk_proof: String,
    ) -> Result<(), Box<dyn std::error::Error>> {
        ...
        let rollup_message = format!(
            "{{\"rollup_proof\": \"{}\", \"zk_proof\": \"{}\"}}",
            rollup_proof, zk_proof
        );
