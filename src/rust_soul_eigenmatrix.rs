...
impl RustSoulEigenmatrix {
    ...
    pub fn extract_from_cargo_lock(lock_content: &str) -> Result<Self, String> {
        ...
        let self_compilation_proof =
            Self::create_self_compilation_proof(&compilation_trace, &eigenvalues)?;
    ...
    fn generate_compilation_trace(
        deps: &HashMap<String, Vec<String>>,
    ) -> Result<Vec<CompilationStep>, String> {
    ...
    fn create_self_compilation_proof(
        trace: &[CompilationStep],
        eigenvalues: &[f64],
    ) -> Result<SelfCompilationProof, String> {
        // Phase 1: Bootstrap
