...
impl SystemArg {
    ...
    fn parse_orbit_index(orbit_part: &str) -> Result<u32, String> {
        if orbit_part.starts_with('a') {
            orbit_part[1..]
                .parse()
                .map_err(|_| "Invalid orbit index".to_string())
        } else {
    ...
    pub fn orbit(&self) -> &LmfdbOrbit {
        match self {
            ...
            | SystemArg::Regex(o)
            | SystemArg::Git(o)
            | SystemArg::Blockchain(o)
            | SystemArg::ZkProof(o)
            | SystemArg::Enterprise(o)
            | SystemArg::Security(o)
            | SystemArg::DataFlow(o)
            | SystemArg::Knowledge(o)
...
impl SystemInstance {
    ...
    pub fn signature(&self) -> String {
        let total_weight: u32 = self.orbits.iter().map(|o| o.orbit().weight).sum();
        let total_dimension: u32 = self.orbits.iter().map(|o| o.orbit().dimension).sum();
