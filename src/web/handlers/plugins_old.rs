...
use crate::web::handlers::{value_lattice_plugin::value_lattice_plugin, process_monitor_plugin::process_monitor_plugin};
...
fn setup_plugins() -> Arc<PluginRegistry> {
    ...
    registry.register_plugin(WebPlugin {
        ...
        handler: Arc::new(Box::new(|| {
            match generate_value_lattice_html() {
                Ok(html) => html,
                Err(e) => {
                    eprintln!("Value lattice plugin error: {}", e);
                    Response::builder()
                        .status(StatusCode::INTERNAL_SERVER_ERROR)
                        .header(header::CONTENT_TYPE, "text/html; charset=utf-8")
                        .body(format!("<h1>Error</h1><p>{}</p>", e))
                        .unwrap()
                }
            }
        })),
...
fn generate_value_lattice_html() -> Result<Response<String>, String> {
