...
use crate::notebooklm_interface::{NotebookLMInterface, ChunkType};
use std::fs;
...
impl NotebookLMCLI {
    ...
    pub fn import_file(&mut self, file_path: &str, topic: &str) -> Result<String, String> {
        ...
        let chunk_id = self.interface.create_intelligent_chunk(content, topic.to_string())?;
        ...
        for entry in entries {
            ...
            if path.is_file() {
                if let Some(file_name) = path.file_name().and_then(|n| n.to_str()) {
                    if file_name.ends_with(".txt") || file_name.ends_with(".md") || file_name.ends_with(".rs") {
